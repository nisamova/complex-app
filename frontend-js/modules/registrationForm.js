...
export default class RegistrationForm {
  ...
  //Methods
  formSubmitHandler() {
    this.usernameImmediatley()
    this.usernameAfterDelay()
    this.emailAfterDelay()
    this.passwordImmediatley()
  ...
  passwordAfterDelay() {
    if (this.password.value.length < 12) {
      this.showValidationError(this.password, "Password must 12 characters")
    }
  }
  ...
  usernameImmediatley() {
    ...
    if (this.username.value.length > 30) {
      this.showValidationError(
        this.username,
        "Username cannot exceed 30 charcaters"
      )
    }
  ...
  usernameAfterDelay() {
    if (this.username.value.length < 3) {
      this.showValidationError(
        this.username,
        "Username must be at least 3 charcaters."
      )
    }
    if (!this.username.errors) {
      axios
        ...
        .then(response => {
          if (response.data) {
            this.showValidationError(this.username, "That user name is taken")
            this.username.isUnique = false
          } else {
